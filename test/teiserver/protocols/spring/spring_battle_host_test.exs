defmodule Teiserver.SpringBattleHostTest do
  ...
  import Teiserver.TeiserverTestLib,
    only: [auth_setup: 1, _send_raw: 2, _recv_raw: 1, _recv_until: 1]
  ...
  @tag :needs_attention
  test "!rehost bug test", %{socket: host_socket, user: host_user} = opts do
    %{socket: watcher_socket} = auth_setup(opts)
    %{socket: p1_socket, user: p1_user} = auth_setup(opts)
    %{socket: p2_socket, user: p2_user} = auth_setup(opts)
  ...
  @tag :needs_attention
  test "host battle test", %{socket: socket, user: user} = opts do
    _send_raw(
      socket,
      "OPENBATTLE 0 0 empty 322 16 gameHash 0 mapHash engineName\tengineVersion\tlobby_host_test\tgameTitle\tgameName\n"
    )
    ...
    %{socket: socket2, user: user2} = auth_setup(opts)
    ...
    # Add user 3
    %{socket: socket3, user: user3} = auth_setup(opts)
